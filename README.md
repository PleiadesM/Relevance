...
- **Themes:** Three themes, each adapting a well-known open design — `the-type` (typography-first serif, the most functional; inspired by [The Type](https://www.thetype.com/)), `papermod` (clean system-sans entry cards; adapts [hugo-PaperMod](https://github.com/adityatelange/hugo-PaperMod)), and `blowfish` (lowkey violet with a blurred sticky nav; adapts [Blowfish](https://github.com/nunocoracao/blowfish)). Every theme ships a designed dark variant — follows your system by default, Light/Dark/Auto override in Settings, ☀/☾ toggle in the header.
