...
| Drive-by setup issue on your public repo | The apply workflow runs **only** when the repository owner is **both the issue author and the account that fired the event**. `edited`/`labeled` re-run the job with the issue body as input, so an author-only check would let anyone with triage/write access edit the owner's issue and have the bot apply *their* content. The body reaches the parser via env→file (no shell/template interpolation); a credential-pattern scanner rejects bodies that look like they contain secrets |
| Setup form asked for private mode before a passphrase exists | The workflow probes whether `NEWSDASH_PASSPHRASE` is set — emitting a boolean only, never the value — and the parser refuses the form instead of committing a config every build would reject. It never silently downgrades the request to public. `scripts/build.py` remains the authoritative hard fail |
