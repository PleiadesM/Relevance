...
- **快车道（10 分钟，零密钥）** → 做完第 1–3 步后直接跳到第 5 步，跳过第 4 步。你会得到一个自动更新的新闻 + 论文仪表盘。
- **完整配置（再加约 10 分钟）** → 加上第 4 步（口令 / 私密模式）和第 6 步（学术论文包）。
...
**可跳过。** 如果你只想要一个公开仪表盘，直接跳到[第 5 步](#5-用配置-issue-个性化)——这一步只在你想要私密模式时才需要。
...
> ⚠️ **绝不要把密钥粘进 Issue**——口令、Token 都不行。机器人会主动扫描疑似凭据的字符串，发现即拒绝应用。密钥只去 **Settings → Secrets and variables → Actions**（[第 4 步](#4-私密模式与口令)）。另外：只有当**仓库所有者既是该 Issue 的作者、又是触发本次运行的人**时，机器人才会应用表单。别人在你仓库开的配置 Issue 会被直接忽略；协作者编辑*你的*配置 Issue 同样不会触发——否则任何有写权限的人都能换成自己的答案让机器人照单执行。

## 6. 学术论文包

在配置 Issue（[第 5 步](#5-用配置-issue-个性化)）里勾选**数据可视化**和/或**技术传播**即可——不用改任何代码。它们走免密钥学术 API，可靠性各有差异：
